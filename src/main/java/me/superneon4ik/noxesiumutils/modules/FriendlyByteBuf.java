...
public class FriendlyByteBuf extends ByteBuf {
    ...
    public FriendlyByteBuf writeVarIntArray(List<Integer> is) {
        this.writeVarInt(is.size());
        for(int i: is) {
            this.writeVarInt(i);
        }
    ...
    public int[] readVarIntArray(int i) {
        int j = this.readVarInt();
        if (j > i) {
            throw new RuntimeException("VarIntArray with size " + j + " is bigger than allowed " + i);
        } else {
            int[] is = new int[j];
    ...
    public long[] readLongArray(@Nullable long[] ls, int i) {
        ...
        if (ls == null || ls.length != j) {
            if (j > i) {
                throw new RuntimeException("LongArray with size " + j + " is bigger than allowed " + i);
            }
